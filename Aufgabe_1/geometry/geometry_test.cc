...
TEST(SPHERE, Inside_Center) {
  // Eine Kugel mit Radius 5 im Ursprung
  Sphere3df sphere = { {0.0f, 0.0f, 0.0f}, 5.0f };
  
  // Der Mittelpunkt selbst sollte "inside" sein
  EXPECT_TRUE( sphere.inside( Vector3df{0.0f, 0.0f, 0.0f}) );
}

TEST(SPHERE, Inside_Boundary) {
  // Eine Kugel mit Radius 3, verschoben
  Sphere2df sphere = { {10.0f, 10.0f}, 3.0f };
  
  // Ein Punkt exakt auf der Oberfläche (z.B. bei x = 13)
  // (13-10)^2 + (10-10)^2 = 3^2. Sollte noch als "inside" zählen.
  EXPECT_TRUE( sphere.inside( Vector2df{13.0f, 10.0f}) );
}

TEST(SPHERE, Intersects_CompletelyInside) {
  // sphereB ist komplett in sphereA enthalten
  Sphere2df sphereA = { {0.0, 0.0}, 10.0 };
  Sphere2df sphereB = { {1.0, 1.0}, 2.0 };
  
  EXPECT_TRUE( sphereA.intersects(sphereB) );
  EXPECT_TRUE( sphereB.intersects(sphereA) ); // Test in beide Richtungen
}

TEST(SPHERE, Intersects_TouchingBoundary) {
  // Zwei Kugeln, die sich exakt an einem Punkt berühren
  // Radius A = 5, Radius B = 3. Abstand der Zentren = 8.
  Sphere2df sphereA = { {0.0, 0.0}, 5.0 };
  Sphere2df sphereB = { {8.0, 0.0}, 3.0 };
  
  EXPECT_TRUE( sphereA.intersects(sphereB) );
}

TEST(SPHERE, Intersects_JustBarelyNotTouching) {
  // Wie oben, aber der Abstand ist 8.001
  Sphere2df sphereA = { {0.0, 0.0}, 5.0 };
  Sphere2df sphereB = { {8.001, 0.0}, 3.0 };
