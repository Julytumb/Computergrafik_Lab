...
bool Sphere<FLOAT,N>::inside(const Vector<FLOAT, N> p) const {
  // Ein Punkt ist "inside", wenn sein quadrierter Abstand zum Zentrum
  // kleiner oder gleich dem quadrierten Radius ist.
  // (p - center).square_of_length() ist (Abstand)^2
  return (p - center).square_of_length() <= (radius * radius);
}
